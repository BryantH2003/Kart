...
### Cache operations are never fatal — always use .catch()
Any call to a cache read or write (`cacheRepo.get()`, `cacheRepo.set()`, or equivalent) must be wrapped with `.catch()`. A cache miss or write failure is an acceptable degradation — the operation proceeds without caching. Never let a cache layer failure surface as a user-visible error. This applies to all caching layers in the project (`search_cache`, any future Redis/KV layer).
...
### Server Components call services directly — never fetch their own API routes
Server Components must import and call service functions directly (e.g. `search()`, `getProductPage()`, `browse()`). Never use `fetch()` to call an internal `/api/` route from a Server Component. The HTTP API exists for external callers; a Server Component going through HTTP adds a round-trip, a failure mode, and an env var dependency (`NEXT_PUBLIC_SITE_URL`) with no benefit. If a service function doesn't exist yet, create it — do not reach into the route handler.
...
### All fetch() calls to CheapShark must include a User-Agent header
CheapShark sits behind Cloudflare. Without a `User-Agent`, requests from cloud provider IPs (Railway, AWS, GCP, Vercel) are blocked with a 403. Node.js `fetch()` sends no `User-Agent` by default. Use the shared `FETCH_HEADERS` constant defined at the top of the adapter — never call `fetch(url)` bare in this file.

---

## Vendor Adapters

### All vendor fetch() calls must set explicit headers
Node.js `fetch()` sends no `User-Agent` by default. CDN-level bot protection (Cloudflare, Fastly, etc.) will block requests from cloud IPs that arrive headerless. Every new adapter must define its own headers constant and pass it to every `fetch()` call. At minimum: `User-Agent` and `Accept: application/json`.
